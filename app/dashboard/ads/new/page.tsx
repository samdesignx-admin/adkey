...
export default function NewAd(){
 ...
 async function publish(){
  ...
  const url=window.location.origin+"/a/"+code,qr=await QRCode.toDataURL(url,{margin:1,width:512});setResult({code,url,qr});setLoading(false);setStep(4);\n  if(user.email){fetch("/api/email/adkey-live",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:user.email,adTitle:title.trim(),code,url,qr})}).catch(()=>{});}
 }
