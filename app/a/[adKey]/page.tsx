...
export default async function PublicAd({params}:{params:Promise<{adKey:string}>}){
 ...
 <div style={{display:"grid",gap:16,marginTop:24}}>{signed.length?signed.map(item=>item.url&&<div className="media" key={item.path}>{item.type==="video"?<video src={item.url} controls playsInline style={{width:"100%",display:"block"}}/>:<img src={item.url} alt={item.name||ad.title} style={{width:"100%",display:"block"}}/>}</div>):<div className="media">Advertisement media coming soon</div>}</div>
